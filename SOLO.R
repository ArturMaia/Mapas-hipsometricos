...
plot(pa$geom)+
  plot(topo,add=T,col=cores,legend.width= 1.5,axis.args=list(cex.axis=0.6),
       legend.args=list(text="Altitude (mt)",
                        side=3, line=0.5, cex=0.8))+title('Hipsometria do Estado do Pará')


#MUNICIPIO
file.choose()
img<-raster("C:\\Users\\luiza\\OneDrive\\Documentos\\raster\\ananin.tif")
ananindeua<-mun<-read_municipality(code_muni = 1500800)
